...
if len(sys.argv) != EXPECTED_ARGUMENTS:
    ...
    fail(
        "Invalid number of arguments received from C#."
    )
...
if penalty_percent > 100:
    ...
    fail(
        "Penalty Percentage cannot be greater than 100."
    )
...
if (
    breach_responsibility
    not in ALLOWED_RESPONSIBILITIES
):

    fail(
        "Invalid breach responsibility value."
    )
...
try:
    ...
    fail(
        f"Failed to open workbook for validation.\n{str(error)}"
    )
...
try:
    ...
    if row_number > sheet.max_row:
        ...
        fail(
            "Contract row does not exist."
        )



    if safe_string(
        values_sheet[f"B{row_number}"].value
    ) == "":

        fail(
            "No contract exists in selected row."
        )
    ...
    if dynamic_status == "Expired":
        ...
        fail(
            "Expired historical contracts cannot be edited."
        )
    ...
    if (
        not breach_detected
        and breach_responsibility != "Pending"
    ):
    ...
    if breach_detected:
        ...
        sheet[f"R{row_number}"] = (
            breach_responsibility
        )

    elif safe_string(
        sheet[f"R{row_number}"].value
    ) == "":
